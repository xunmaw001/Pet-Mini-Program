<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.dianmianzhaopian" label="店面照片" prop="dianmianzhaopian">
          <file-upload
          tip="点击上传店面照片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.dianmianzhaopian?ruleForm.dianmianzhaopian:''"
          @change="dianmianzhaopianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.dianmianzhaopian" label="店面照片" prop="dianmianzhaopian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.dianmianzhaopian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="店名" prop="dianming">
          <el-input v-model="ruleForm.dianming" 
              placeholder="店名" clearable  :readonly="ro.dianming"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="店名" prop="dianming">
              <el-input v-model="ruleForm.dianming" 
                placeholder="店名" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="具体地址" prop="jutidizhi">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.jutidizhi" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.jutidizhi" label="具体地址" prop="jutidizhi">
                    <span v-html="ruleForm.jutidizhi"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `fujinchongwudianhuoyiyuan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.jutidizhi = this.ruleForm.jutidizhi.replace(reg,'../../../ssm91oey/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `fujinchongwudianhuoyiyuan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.fujinchongwudianhuoyiyuanCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    dianmianzhaopianUploadChange(fileUrls) {
	this.ruleForm.dianmianzhaopian = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
