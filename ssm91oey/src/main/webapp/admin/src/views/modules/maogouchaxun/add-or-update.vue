<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="品种名" prop="pinzhongming">
          <el-input v-model="ruleForm.pinzhongming" 
              placeholder="品种名" clearable  :readonly="ro.pinzhongming"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="品种名" prop="pinzhongming">
              <el-input v-model="ruleForm.pinzhongming" 
                placeholder="品种名" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="大概价格" prop="dagaijiage">
          <el-input v-model="ruleForm.dagaijiage" 
              placeholder="大概价格" clearable  :readonly="ro.dagaijiage"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="大概价格" prop="dagaijiage">
              <el-input v-model="ruleForm.dagaijiage" 
                placeholder="大概价格" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="特征描述" prop="tezhengmiaoshu">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.tezhengmiaoshu" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.tezhengmiaoshu" label="特征描述" prop="tezhengmiaoshu">
                    <span v-html="ruleForm.tezhengmiaoshu"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zhaopian = true;
          ...
          if(o=='pinzhongming'){
            this.ruleForm.pinzhongming = obj[o];
	    this.ro.pinzhongming = true;
            continue;
          }
          if(o=='dagaijiage'){
            this.ruleForm.dagaijiage = obj[o];
	    this.ro.dagaijiage = true;
            continue;
          }
          if(o=='tezhengmiaoshu'){
            this.ruleForm.tezhengmiaoshu = obj[o];
	    this.ro.tezhengmiaoshu = true;
            continue;
          }
          if(o=='jieshao'){
	...
	this.ruleForm.tezhengmiaoshu = this.ruleForm.tezhengmiaoshu.replace(reg,'../../../ssm91oey/upload');
	this.ruleForm.jieshao = this.ruleForm.jieshao.replace(reg,'../../../ssm91oey/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `maogouchaxun/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.maogouchaxunCrossAddOrUpdateFlag = false;
                  this.parent.search();
